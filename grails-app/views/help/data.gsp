...
<div id="help_content">
    ...
    <p>
        ...
        dynamically using a <a
            href="http://www.biocase.org/products/provider_software/">BioCASe provider</a>, which provides new and updated records to ${shortName} on a daily
    basis.
    </p>
    ...
    <p>
        ...
        overview of the provided versus processed data is available for each record via the <a
            href="help.html#record_detail"><span class="feature">Record detail</span></a> page.
    </p>

    <p>
        Australia’s Seedbank Program contains records of the holdings of Australian herbaria, both of Australian and foreign
    ...
    used. ASBP records may also just be wrong. A record in ASBP of a species from a certain location only means that
    there is a specimen with this species name from the given location, not necessarily that the species naturally
    occurs at that location. The major asset of ASBP is that all records are based on preserved specimens, so they can,
    to a large extent, be verified. It is up to the user to decide whether or not verification is necessary. By itself
    ASBP data is not more authoritative than data you get from anywhere else.
    </p>
    ...
    <p>
        The <a
            href="http://sweetgum.nybg.org/ih/">Index Herbariorum</a> code of the herbarium at which the specimen is held.
    </p>
    ...
    <p>
        An identifier that identifies the physical specimen. In ${shortName}, catalogue numbers consist of the <span
            class="feature">Herbarium code</span>, followed by a space and then the catalogue
    ...
    <p>
        The unique identifier applied to the specimen record in the ALA BioCache. The <span
            class="feature">ALA record ID</span> is listed in the <a href="help.html#downloads"><span
            class="feature">CSV downloads</span></a> and in the URL of the <a href="help.html#record_detail"><span
            class="feature">Record detail</span></a> page. You can use the <span
            class="feature">ALA record ID</span> to find the most up-to-date data for the specimen record by typing '${hostname}/[record
    ...
    <p>
        The date the record was last updated. On the <a href="help.html#record_detail"><span
            class="feature">Record detail</span></a> pages this is shown as <span
            class="feature">Date loaded</span> and will appear under the location map. The <span
            class="feature">Date last processed</span> indicates when the record was last processed within the ALA BioCache. This may happen, for instance, when there is a change in the backbone
    ...
    <p>
        The date that the specimen was collected. Collecting dates in ${shortName} are delivered in two ways: as <span
            class="feature">Collecting date</span>, which has to be ISO compliant, or as
        <span class="feature">Verbatim collecting date</span>. Some herbaria deliver <span
            class="feature">Verbatim collecting date</span> only when an ISO-compliant <span
            class="feature">Collecting date</span> can't be delivered, or not all information can be delivered as an ISO-compliant date (e.g. 'Summer of '69'), while others deliver it for all collecting dates. Because of database
    restrictions, some herbaria are only able to deliver incomplete collecting dates as <span
            class="feature">Verbatim collecting date</span>. We hope to remedy this in the near future.
    </p>
    ...
    <p>
        The <span class="feature">Collecting date</span> term in the <a href="help.html#advanced_search"><span
            class="feature">Advanced search</span>
    </a> form allows you to query for a range of collecting dates. If you enter just a start date or an end date,
    ...
    <p>
        There are <span class="feature">Year</span>, <span class="feature">Month</span> and <span
            class="feature">Decade</span> facets on the <a href="help.html#results"><span class="feature">Results</span>
    </a> page that allow you to filter your results for records collected in a particular year, month or
    decade. Note that the <span
            class="feature">Decade</span> facet and chart only include records with complete collecting dates; records with only a month and a year, or just a year,
    will not be counted.
    </p>
    ...
    <p>
        ...
        fields for other types of notes have not always had them &ndash; so, for many records, <span
            class="feature">Collecting notes</span> will contain more than just collecting notes.
    </p>
    ...
    <p>
        The <span
            class="feature">Phenology</span> field indicates whether the specimen is bearing flowers or fruits, etc. Only two herbaria, MEL and NSW, deliver this information.
    </p>
    ...
    <p>
        ...
        called <span
            class="feature">Taxon name (provided)</span> in ${shortName} &ndash; but the taxon name will also be processed. The processing of the taxon name includes parsing the name into its parts,
    ...
    <p>
        If a matched name is in one of the authoritative national checklists that are part of the ALA name list, such as the <a
            href="http://www.anbg.gov.au/chah/apc/">Australian Plant Census</a> (APC), and if
    the matched name is considered a synonym, the processed name will be the accepted name from the checklist. The processed taxon name is called <span
            class="feature">Taxon name (processed)</span> in ${shortName}.
    </p>

    <p>
        The <span
            class="feature">Taxon rank (matched)</span> is the rank of the processed taxon name. This may be the same rank as the rank of the provided name, or a higher rank. If the rank
    ...
    provided name will be given on the <a href="help.html#record_detail"><span class="feature">Record detail</span>
    </a> page as well.
    </p>

    <p>
        The <span
            class="feature">Name match metric</span> describes how the provided name was matched to a name in the ALA name list. If the parsed name was matched, the <span
            class="feature">Name match metric</span>
    ...
    <p>
        <span class="feature">Author</span> is the authorship of the processed name. The authorship of the provided name is given as part of <span
            class="feature">Taxon name (provided)</span>.
    </p>

    <p>
        The <span
            class="feature">Common name</span> is the common name recorded in the ALA name list. Common names are never provided by the herbaria.
    </p>
    ...
    <p>
        There are facets for taxonomic groups of all mandatory ranks &ndash; <span class="feature">Kingdom</span>, <span
            class="feature">Phylum</span>, <span class="feature">Class</span>, <span class="feature">Order</span>, <span
            class="feature">Family</span>, <span class="feature">Genus</span>, <span
            class="feature">Species</span> &ndash; and for <span class="feature">Infraspecific taxon</span>. These
    taxonomic groups are also given on the <a href="help.html#record_detail"><span class="feature">Record detail</span>
    </a> page and in the <a href="help.html#downloads"><span class="feature">CSV downloads</span>
    </a>. If there was no match for a provided name, the names of the taxonomic
    ...
    <p>
        The <span
            class="feature">Botanical group</span> facet allows you to select records of a botanical group that is not necessarily a taxonomic group, for example, bryophytes, angiosperms
    ...
    <p>
        The <span
            class="feature">Determination qualifier</span> facet can be used to select records with certain determination qualifiers, or to exclude records with uncertain determination.
    ...
    <p>
        The <span class="feature">Determination date</span> term in the <a href="help.html#advanced_search"><span
            class="feature">Advanced search</span>
    </a> allows you to query for a range of determination dates. If you fill in just a start date or an end date, your results
    ...
    <p>
        ...
        a record, you can flag an issue on the <a href="help.html#record_detail"><span
            class="feature">Record detail</span></a> page.
    </p>
    ...
    <p>
        ...
        or the latitude and longitude are transposed, the values are corrected and the unprocessed latitude and longitude are given in the <span
            class="feature">Latitude (provided)</span> and
        <span class="feature">Longitude (provided)</span> fields in the <a href="help.html#record_detail"><span
            class="feature">Record detail</span></a> page and in downloaded results.
    </p>
    ...
    <p>
        ...
        different datums. <span
            class="feature">Geodetic datum</span> is very haphazardly delivered with ${shortName} data, even with georeferences that are accurate enough for the geodetic datum to be
    meaningful.
    </p>
    ...
    <p>
        The <span
            class="feature">Geocode uncertainty</span> is a measurement or estimate of how far away in metres the point represented by the latitude and longitude may be from the actual
    ...
    <p>
        ...
        georeference. In ${shortName}, the terms that refer to the person who provided the georeference are stored under <span
            class="feature">Georeferenced by</span>.
    </p>
    ...
    <p>
        The altitude in metres of the collecting locality, if provided with the specimen record. Altitude is presented in ${shortName} as <span
            class="feature">Minimum altitude (m)</span> and <span
            class="feature">Maximum altitude (m)</span>. If a single value for altitude is provided, which will mostly be the case, this will be given as <span
            class="feature">Minimum altitude (m)</span>. If a range is provided a
    ...
    <p>
        The depth in metres where the specimen was collected. Depth is presented in ${shortName} as <span
            class="feature">Minimum depth (m)</span> and <span
            class="feature">Maximum depth (m)</span>. If a single value for depth is provided, which will mostly be the case, this will be given as <span
            class="feature">Minimum depth (m)</span>. If a range is provided a <span
            class="feature">Maximum depth (m)</span> will also be given.
    </p>
    ...
    <p>
        The <a
            href="http://spatial.ala.org.au/layers/more/ibra_merged">Interim Biogeographic Regionalisation for Australia (IBRA) region</a> that corresponds to the latitude and longitude provided with the specimen record.
    </p>
    ...
    <p>
        The <a
            href="http://spatial.ala.org.au/layers/more/imcra_meso">Integrated Marine and Coastal Regionalisation of Australia (IMCRA) meso-scale
    ...
    <p>
        The combined IBRA and IMCRA biogeographic regions. This facet is hidden by default, but can be selected under the <a
            href="help.html#refine_results"><span class="feature">Refine results</span></a> options.
    </p>
    ...
    <p>
        The ecoregion (non-marine, marine, or limnetic) in which the taxon occurs. The data comes from the <a
            href="http://www.obis.org.au/irmng/">Interim Register of Marine and Nonmarine Genera (IRMNG)</a> and is based on the genus name, not from the latitude and longitude associated with the record. Not all genus names in ${shortName} are in IRMNG.
    </p>
    ...
    <p>
        The Major Vegetation Groups (from the <a
            href="http://www.environment.gov.au/erin/nvis/mvg/">National Vegetation Information System</a>)
    ...
    vegetation type (<span
            class="feature">Vegetation types: extant</span>) and the estimated vegetation before European settlement(<span
            class="feature">Vegetation types: pre-1750</span>).
    </p>
    ...
    <p>
        The herbaria to which duplicates have been sent. Herbaria are identified by their <a
            href="http://sweetgum.nybg.org/ih/">Index Herbariorum</a>
        ...
        received the duplicates. A potential use case of the <span
            class="feature">Duplicates sent to</span> query term in the <a href="help.html#advanced_search"><span
            class="feature">Advanced search</span>
    </a> would be a herbarium trying to find the original records of specimens they have received on exchange from all other Australian herbaria, or, if used in combination with the <span
            class="feature">Herbarium field</span>, from one particular Australian herbarium.
    </p>
    ...
    <p>
        The <span
            class="feature">Loan number</span> is the reference number or identifier assigned to the loan by the lending institution, and is used by the lender and the borrower for
    ...
    <p>
        ...
        changes made during processing can be viewed by clicking on the <a href="help.html#original_vs_processed"><span
            class="feature">Original vs Processed</span></a> button on the <a href="help.html#record_detail"><span
            class="feature">Record detail</span>
    </a> page. Users can also flag potential issues with specimen records by using the <a
            href="help.html#flag_an_issue"><span class="feature">Flag an issue</span></a> feature on the <a
            href="help.html#record_detail"><span class="feature">Record detail</span>
    </a> page. Data issues detected during processing or flagged by users are available as a facet on the <a
            href="help.html#results"><span class="feature">Results</span>
    </a> page, and can be used to narrow down your search results. The range of data issues in ${shortName} are described in Table 3. The data issues are also available on the <a
            href="help.html#record_detail"><span class="feature">Record detail</span></a> page and in the <a
            href="help.html#downloads"><span class="feature">CSV downloads</span></a>.
    </p>
    ...
    <div>
        <table>
            ...
            <tr>
                ...
                <td>
                    The value provided in the <span
                        class="feature">Basis of record</span> field could not be mapped against the standard vocabulary used by ALA.
                </td>
            </tr>
            <tr>
                <td>
            ...
            <tr>
                ...
                <td>
                    The <span
                        class="feature">Taxon name (provided)</span> is not in the national species lists for the country in which it was collected, but it is in other checklists,
            ...
            <tr>
                ...
                <td>
                    The <span
                        class="feature">Taxon name (provided)</span> cannot be located on any national or international species checklists.
                </td>
            </tr>
            <tr>
                <td>
            ...
            <tr>
                ...
                <td>
                    ...
                    <br/>
                    Not all records flagged with this issue will be in error; check the <span
                        class="feature">Geocode uncertainty</span> and <span
                        class="feature">Locality</span> fields to see if the record is
    ...
    <p>
        An assessment of whether or not the location is spatially valid, based on a range of data quality checks and user-contributed annotations. If the record suffers from one or more of the geospatial <span
            class="feature">Data issues</span> listed in Table 2 it is considered 'Spatially suspect', otherwise it is 'Spatially valid'. Note that a 'Spatially valid' record is not necessarily correctly georeferenced, and, depending on
    ...
    <p>
        ...
        the notes on the <a
            href="http://code.google.com/p/ala-dataquality/wiki/DETECTED_OUTLIER_JACKKNIFE">spatial outlier detection method</a> used by ALA.
    </p>

    <p>
        The <span
            class="feature">Outlier for layer</span> facet indicates if a specimen is an outlier for an environmental layer, based on the known environmental range of the taxon to which
    the specimen has been identified. The <span
            class="feature">Outlier layer count</span> facet allows you to filter your results for records that are outliers for certain numbers of
    environmental layers. You can also display your results by <span class="feature">Outlier for layer</span> or <span
            class="feature">Outlier layer count</span> on the distribution map. If a record is an outlier for one or
    more layers, the <a href="help.html#record_detail"><span class="feature">Record detail</span>
    </a> page will display graphs for each of the variables for which the record is an outlier with the distribution of the records
    of the taxon to which it belongs. The layers for which a record is an outlier will also be given in the <a
            href="help.html#downloads"><span class="feature">CSV downloads</span></a>.
    </p>
    ...
    <p>
        ...
        (<i>Wollemia nobilis</i>). There is a <span
            class="feature">Sensitive data</span> facet that can be used to check for sensitive data among query results. The <a
            href="help.html#record_detail"><span class="feature">Record detail</span></a> page and the <a
            href="help.html#downloads"><span class="feature">CSV downloads</span>
    </a> indicate whether distribution data has been withheld or generalised for each record.
    </p>
    ...
    <p>
        The <span
            class="feature">Multimedia</span> facet allows you to filter your results for records that have images or other multimedia attached. Currently, there are no records with
